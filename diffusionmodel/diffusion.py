...
class GaussianDiffusion(LightningModule):
    def __init__(self, T, dynamic_threshold=False) -> None:
        super().__init__()
        self.betas = linear_beta_schedule(timesteps=T).to('cuda:0')
    ...
    def forward_diff(self, x_0, t, T=300):
        ...
        #noise = torch.randn_like(ab).to('cuda:0')
        ...
        # noised_img = torch.cat((l, ab_noised), dim=1).to('cuda:0')
    ...
    def sample_timestep(self, model, encoder, x, t, cond=None, T=300, ema=None):
        ...
        # May have to flip the commenting of the fofllowing two lines for training/inference
        # greyscale_emb = encoder(x_l).to(self.device)
